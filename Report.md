...
👉 **`docker-compose.yml` соответствует этому требованию.** ✅
...
fcgi-server:
  ...
  networks:
    - mynetwork
```
- У `fcgi-server` ты написал `expose: 81`.  
  - **`expose`** в `docker-compose` — это просто открытие порта **внутри docker-сети**, чтобы другие контейнеры могли с ним общаться.
  - **`expose` НЕ публикует порт наружу** на твою локальную машину (в отличие от `ports`).
- Ты **НЕ прописал `ports:` для `fcgi-server`**, то есть порт 81 **НЕ мапится на хост** (на твою систему).
...
### 📢 Вывод:
**Ты полностью правильно понял и выполнил эту часть задания. Молодец! 🔥**

---

Хочешь, ещё кратко объясню разницу между `EXPOSE` в `Dockerfile` и `expose` в `docker-compose.yml`? Очень полезно для будущих проектов. 🚀  
(если надо — скажи "да")
...
Хочешь, ещё дополнительно сделаю красивую шпаргалку, как именно выглядел `docker-compose.yml`, чтобы её можно было приложить? 🚀  
(если да — скажи, я быстро соберу)