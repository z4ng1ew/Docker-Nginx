...
- То есть, если мы попытаемся достучаться до `localhost:80` или `0.0.0.0:80` на хосте — **ничего не выйдет**.
- Этот порт доступен **только внутри контейнера** или для других контейнеров в той же Docker-сети. 

> #### То есть контейнер может "жить своей жизнью", но мы не сможем к нему подключиться извне (из браузера, curl и т.д.).
...
То есть: мы можем открыть в браузере http://localhost:8080 и попасть в контейнер.
...
- ![Информация о статусе nginx на странице /status](img/nginx_conf_5.png)  
...
- 🌐 Ограничение/блокировка доступа (например, в России к DockerHub часто ограничен доступ, поэтому в ходе выполнения задания пришлось использовпть зеркала (Mirror));
...
**Описание:**
На этом скриншоте я скачал самую последнюю стабильную версию Docker Compose вручную.  
...
## 2. 🛠️ Сделал скачанный файл исполняемым

**Описание:**

После скачивания я задал права на выполнение для файла `docker-compose` с помощью команды `chmod +x docker-compose`.  
...
**Описание:**
Проверил установленную версию Docker Compose командой `docker-compose --version`, чтобы убедиться в правильности установки.  
На скриншоте видно, что установилась актуальная версия.

**Скриншот:**

![alt text](img/версия_докер_компоуз.png)
...
## 4. 📚 Содержание моего Docker compose yaml


**Скриншот:**
![alt text](img/Docker_compose_yaml.png)



👉 **`docker-compose.yml` соответствует требованиям.** ✅



**Требование:**  
> "он должен работать в локальной сети, т.е. не нужно использовать инструкцию `EXPOSE` и мапить порты на локальную машину."
...
fcgi-server:
  ...
  expose:
    - "81"
  command: ["spawn-fcgi", "-p", "81", "-n", "--", "/app/server/fcgi_server"]
  networks:
    - mynetwork
```
- У `fcgi-server` написал `expose: 81`.  
  - **`expose`** в `docker-compose` — это просто открытие порта **внутри docker-сети**, чтобы другие контейнеры могли с ним общаться.
  - **`expose` НЕ публикует порт наружу** на мою локальную машину (в отличие от `ports`).
- Я **НЕ прописал `ports:` для `fcgi-server`**, то есть порт 81 **НЕ мапится на хост** (на твою систему).

---

### 👉 Итог:

| Что требовалось          | Что сделано                | Всё ок? |
|:--------------------------|:------------------------------|:-------|
| Работать только внутри сети | Только `expose`, без `ports`  | ✅ Да  |
| Не пробрасывать наружу     | Порты наружу не мапятся       | ✅ Да  |

---

### 📢 Вывод:
**Всё полностью правильно**
...
## 5. Остановил все запущенные контейнеры.

![alt text](img/docker_stop_6.png)




## 5. 🧱 Сборка и запуск проекта через docker-compose

**Описание:**
Создал файл `docker-compose.yml`, в котором описал:
- Первый контейнер (мой FastCGI сервер), который работает в локальной сети Docker без публикации портов наружу.
- Второй контейнер (nginx), который проксирует запросы с 8080 порта на 81 порт первого контейнера.
- Замапил порт 8080 второго контейнера на порт 80 локальной машины.
...
docker-compose up
```
Эти команды собрали и запустили весь проект через `docker-compose`.

**Скриншот:**

![alt text](img/СБОРКА_И_ЗАПУС_ДОКЕР_КОМПОУЗ.png)
...
**Описание:**
Открыл браузер и перешёл на `http://localhost:80`.  
На странице отобразилась моя веб-страничка, которую я разрабатывал в предыдущих частях. Это доказывает, что проксирование через nginx и взаимодействие контейнеров работают корректно.

**Скриншот:**

![alt text](img/DOCKER_COMPOSE_FINAL_RESULT.png)

---

# 🎯 Вывод:

✅ Установка и настройка Docker Compose выполнена.  
✅ Проект из нескольких контейнеров успешно собран и запущен через `docker-compose`.  
✅ Страница доступна по адресу `localhost:80` — всё работает как требуется по заданию!
